...
#[tauri::command]
pub async fn run_diagnostics(state: State<'_, AppState>) -> Result<HashMap<String, DiagnosticCheck>, String> {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        checks.insert("accessibility".to_string(), DiagnosticCheck {
            name: "Accessibility Permissions".to_string(),
            status: if has_accessibility { "ok".to_string() } else { "error".to_string() },
            ...
            fix: if !has_accessibility { Some("Open System Settings".to_string()) } else { None },
            fix_action: if !has_accessibility { Some("openAccessibilitySettings()".to_string()) } else { None },
        });
    }
    ...
    match AudioCapture::list_devices() {
        Ok(devices) if !devices.is_empty() => {
            checks.insert("audio".to_string(), DiagnosticCheck {
    ...
    checks.insert("whisper_model".to_string(), DiagnosticCheck {
        name: "Whisper Model".to_string(),
        status: if model_downloaded { "ok".to_string() } else { "warning".to_string() },
        message: if model_downloaded {
            "Model downloaded and ready".to_string()
        } else {
            "Model not downloaded yet".to_string()
        },
        fix: if !model_downloaded { Some("Will download on first use".to_string()) } else { None },
        fix_action: None,
    });

    // 4. Platform info
    checks.insert("platform".to_string(), DiagnosticCheck {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        checks.insert("keyboard".to_string(), DiagnosticCheck {
