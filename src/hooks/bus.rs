...
impl EventBus {
    ...
    pub fn new() -> Self {
        log::debug!("Creating EventBus (capacity={})", CHANNEL_CAPACITY);
    ...
    pub async fn register(&self, handler: Arc<dyn HookHandler>) -> anyhow::Result<()> {
        log::debug!("Registering handler '{}' (priority: {})", handler.id(), handler.priority());
        ...
        let pos = handlers.iter().position(|h| h.id() == handler.id()).unwrap();
        log::info!(
            "Registered handler '{}' at position {} (priority: {}, subscriptions: {:?})",
            handler.id(),
            pos,
            handler.priority(),
            handler.subscribed_events()
        );
    ...
    pub async fn unregister(&self, handler_id: &str) -> anyhow::Result<()> {
        log::debug!("Unregistering handler '{}'", handler_id);
        ...
        if handlers.len() < initial_len {
            log::info!("Unregistered handler '{}' ({} handlers remaining)", handler_id, handlers.len());
        } else {
            log::warn!("Handler '{}' not found for unregistration", handler_id);
        }
    ...
    pub async fn emit(&self, mut event: HookEvent) -> HookEvent {
        ...
        log::trace!("Emitting event: {:?}", event_type);
        ...
        if handler_count > 0 {
            log::trace!("Processing event {:?} through {} handlers", event_type, handler_count);
        }

        let mut handlers_processed = 0;
        // Process through handlers
        for handler in handlers {
            // Skip disabled handlers
            if !handler.is_enabled() {
                log::trace!("Skipping disabled handler '{}'", handler.id());
                continue;
            }
            ...
            log::trace!("Event {:?} being handled by '{}'", event_type, handler.id());
            ...
            match handler.handle(&mut event).await {
                ...
                HookResult::Stop => {
                    ...
                    log::debug!(
                        "Event {:?} stopped by handler '{}' ({} handlers processed)",
                        event_type,
                        handler.id(),
                        handlers_processed
                    );
                ...
                HookResult::Skip => {
                    log::trace!("Handler '{}' skipped event {:?}", handler.id(), event_type);
                }
            }
        }
    ...
    pub async fn subscribe(&self, event_type: HookEventType) -> broadcast::Receiver<HookEvent> {
        ...
        let sender = subscribers.entry(event_type).or_insert_with(|| {
            log::trace!("Created new broadcast channel for {:?}", event_type);
    ...
    pub async fn clear(&self) {
        log::debug!("Clearing all handlers from EventBus");

        let mut handlers = self.handlers.write().await;

        let count = handlers.len();
        log::info!("Clearing {} handlers", count);

        // Call on_unregister for all handlers
        for handler in handlers.iter() {
            log::trace!("Unregistering handler '{}' during clear", handler.id());
